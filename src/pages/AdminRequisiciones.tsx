...
/** 🔗 URL del backend desde tu .env (C:\Users\Tania\rqmarket\.env)
 * VITE_BACKEND_URL=https://rqmarket-backend-production.up.railway.app
 */
...
export default function AdminRequisiciones() {
  ...
  useEffect(() => {
    if (!API) {
      console.warn("⚠️ VITE_BACKEND_URL no está definido. Revisa tu archivo .env en la raíz del frontend.");
    }
  }, []);
      ...
      partidas: [
        {
          ...
          precio_unitario: precioFinal, // ✅ usamos precioFinal con margen
        },
      ],
    };

    try {
      const response = await fetch(`${API}/api/generar-cotizacion-pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (result.success && result.file) {
        // result.file ya viene como "/pdfs/archivo.pdf"
        window.open(`${API}${result.file}`, "_blank");
      } else {
